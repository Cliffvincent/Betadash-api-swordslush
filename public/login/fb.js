...
function getCookie(email, password, callback) {
  ...
  request({ url: xurl, headers: headers, jar: jar }, (error, response, body) => {
    ...
    request.post({ url: xurl, headers: headers, form: formData, followAllRedirects: true, timeout: 10000, jar: jar }, (error, response, body) => {
      if (error) {
...
exports.index = async (req, res, next) => {
  ...
  getCookie(email, password, (error, fbstate) => {
    if (error) {
